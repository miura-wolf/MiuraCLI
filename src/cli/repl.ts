...
export async function runRepl(opts: { resumeSessionId?: string } = {}): Promise<void> {
...
async function executeInput(
  ...
  if (!match) {
    // Free-text → run as a quick agent task with the context-builder for code/explanation
    session.addUser(input);
    const recent = session.getRecentMessages(10);
    const contextPrompt = recent.length > 0
      ? `Recent session context:\n${recent.map(m => `${m.role}: ${m.content.slice(0, 200)}`).join('\n')}\n\nCurrent task:\n${input}`
      : input;

    const result = await miura.runAgent('context-builder', contextPrompt);
...
async function handleResult(result: CommandResult, session: SessionManager): Promise<void> {
  switch (result.type) {
    case 'error':
...
async function createReadline(
  opts: { history: string[]; completer: (line: string) => [string[], string] },
...
function completeCommand(line: string, registry: CommandRegistry): [string[], string] {
