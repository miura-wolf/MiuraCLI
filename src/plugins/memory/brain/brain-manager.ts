...
import type { Plugin, PluginManifest, PluginHostAPI } from '../../../core/types.js';
...
export const DEFAULT_BRAIN_CONFIG: Required<Omit<BrainManagerConfig, 'projectName'>> = {
...
export class BrainManager implements Plugin {
  ...
  private config: Required<Omit<BrainManagerConfig, 'projectName'>> & { projectName: string };
   ...
   */
  async search(query: string, opts: { limit?: number; type?: BrainEntryType } = {}): Promise<BrainEntry[]> {
   ...
   */
  async save(entry: Omit<BrainEntry, 'id' | 'createdAt' | 'project'>): Promise<BrainEntry> {
   ...
   */
  async saveDecision(title: string, content: string, sessionId?: string): Promise<BrainEntry> {
    return this.save({
      type: 'decision',
   ...
   */
  async saveArchitecture(title: string, content: string, sessionId?: string): Promise<BrainEntry> {
    return this.save({
      type: 'architecture',
      topicKey: title.toLowerCase().replace(/\s+/g, '-'),
      title,
   ...
   */
  async savePattern(title: string, content: string, sessionId?: string): Promise<BrainEntry> {
    return this.save({
      type: 'pattern',
      topicKey: title.toLowerCase().replace(/\s+/g, '-'),
      title,
   ...
   * Get session summary.
   */
  async getSessionSummary(sessionId: string): Promise<{ entries: number; types: Record<string, number> }> {
    const entries = await this.store.getBySession(sessionId, this.config.projectName);
  ...
  async getRelevantContext(query: string, maxEntries = 5): Promise<string> {
    ...
    for (const e of entries) {
      ...
      lines.push(e.content.slice(0, 400) + (e.content.length > 400 ? '...' : ''));
      lines.push('');
    }
    return lines.join('\n');
  }
  ...
  private registerEventListeners(): void {
    ...
    if (this.config.capturePipelineEvents) {
      this.eventBus!.on('pipeline.completed', this.boundHandlers.onCompleted as any);
  ...
  private deregisterEventListeners(): void {
    ...
    this.eventBus.off('pipeline.completed', this.boundHandlers.onCompleted as any);
    this.eventBus.off('pipeline.failed', this.boundHandlers.onFailed as any);
  }

  private onPipelineStuck(payload: { pipelineId: string; detector: StuckSignal }): void {
    const { pipelineId, detector } = payload;
    this.save({
      type: 'discovery',
      topicKey: `stuck-${detector.type}`,
      title: `Pipeline stuck: ${pipelineId}`,
      content: `Pipeline ${pipelineId} got stuck.\n` +
        `Type: ${detector.type}\n` +
      ...
      this.host?.emit('system.error', { error: String(err), source: 'BrainManager.onPipelineStuck' });
    });
  }

  private onPipelineCompleted(payload: { pipelineId: string; result: PipelineResult }): void {
    const { pipelineId, result } = payload;
    this.save({
      type: 'discovery',
      topicKey: `pipeline-${pipelineId}`,
      title: `Pipeline completed: ${pipelineId}`,
      content: `Pipeline ${pipelineId} completed in ${result.iterations} iterations (${result.totalDurationMs}ms).\n` +
      ...
      this.host?.emit('system.error', { error: String(err), source: 'BrainManager.onPipelineCompleted' });
    });
  }

  private onPipelineFailed(payload: { pipelineId: string; error: string }): void {
