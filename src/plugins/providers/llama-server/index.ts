...
export class LlamaServerManager extends EventEmitter {
  ...
  private spawnImpl: (command: string, args: string[], opts: object) => ChildProcess;
  ...
  async start(): Promise<void> {
    ...
    if (existing) {
      this.emit('info', `llama-server already listening on port ${this.config.port}`);
  ...
  async healthCheck(): Promise<LlamaHealthStatus> {
    ...
    try {
      ...
      if (response.ok) {
        ...
        try {
          const data = await response.json() as { gpu_memory_used_mb?: number };
  ...
  private async spawnServer(): Promise<void> {
    const { serverPath, modelPath, port, contextSize, gpuLayers, extraArgs } = this.config;
    ...
    this.process.stdout?.on('data', (data: Buffer) => {
      ...
      if (line.includes('HTTP server listening') || line.includes('server started')) {
    ...
    this.process.on('exit', (code, signal) => {
      clearTimeout(startupTimeout);
      this.emit('server-log', `llama-server exited: code=${code} signal=${signal}`);
      ...
      // Auto-restart on unexpected exit
      if (!this.restarting) {
        this.emit('warn', `llama-server crashed (${signal ?? code}), restarting in 3s...`);
  ...
  private async checkPortInUse(port: number): Promise<boolean> {
    try {
      ...
      const response = await this.fetchImpl(`http://127.0.0.1:${port}/v1/models`, {
        signal: controller.signal,
      });
      clearTimeout(timeout);
      return response.ok;
