...
export function formatTrace(
	result: NonNullable<ReturnType<GraphIndexer["traceCallGraph"]>>,
): string {
	...
	async activate(): Promise<void> {
		if (!this.parsersReady) {
			try {
				...
				// tree-sitter WASM not available — graph features disabled
			}
		}
	}
	...
	async runInit(projectPath: string): Promise<IndexStats> {
		...
		if (!this.parsersReady) {
			try {
				...
				throw new Error(
					`tree-sitter not available. Install tree-sitter WASM packages:
  bun add tree-sitter-typescript tree-sitter-javascript tree-sitter-go tree-sitter-python`,
				);
			}
		}
	...
	private makeGraphSearchTool(): ToolHandler {
		return {
			definition: {
				name: "graph_search",
				description:
					"Full-text search across all indexed symbols (FTS5-powered)",
				parameters: {
					type: "object",
					properties: {
						...
						type: {
							type: "string",
							description:
								"Filter by symbol type: function|class|interface|method|variable",
						},
					},
					required: ["query"],
				},
			...
			execute: async (args) => {
				...
				if (!indexer)
					return {
						name: "graph_search",
						output: "GraphIndexer not initialized",
						durationMs: 0,
					};
	...
	private makeGraphContextTool(): ToolHandler {
		return {
			definition: {
				name: "graph_context",
				description:
					"Get architectural context about a symbol (file, function, class)",
				parameters: {
					type: "object",
					properties: {
						...
						file: {
							type: "string",
							description: "Optional: restrict to file path",
						},
					},
					required: ["symbol"],
				},
			...
			execute: async (args) => {
				...
				if (!ctx)
					return {
						name: "graph_context",
						output: `Symbol "${args.symbol}" not found`,
						durationMs: 0,
					};
				...
				const lines = [
					...
					...neighbors.map(
						(n) =>
							`  ${n.symbolType.padEnd(10)} ${n.symbolName.padEnd(30)} :${n.line}`,
					),
				];
