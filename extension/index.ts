/**
 * MiuraSwarm Extension for Pi
 * 
 * Registers MiuraSwarm as providers in pi:
 * - Ollama (local models)
 * - NVIDIA NIM (cloud models: DeepSeek, Gemma, Kimi, etc.)
 * 
 * Also registers custom tools for pipeline orchestration.
 * 
 * Usage: Place in ~/.pi/agent/extensions/miuraswarm/index.ts
...
const OLLAMA_MODELS = [
  { id: "llama3.1:8b", name: "Llama 3.1 8B (Local)", reasoning: false, input: ["text"], contextWindow: 128_000, maxTokens: 4096 },
  { id: "llama3.1:70b", name: "Llama 3.1 70B (Local)", reasoning: false, input: ["text"], contextWindow: 128_000, maxTokens: 4096 },
  { id: "qwen2.5-coder:7b", name: "Qwen 2.5 Coder 7B (Local)", reasoning: false, input: ["text"], contextWindow: 128_000, maxTokens: 4096 },
  { id: "qwen2.5-coder:14b", name: "Qwen 2.5 Coder 14B (Local)", reasoning: false, input: ["text"], contextWindow: 128_000, maxTokens: 4096 },
  { id: "codellama:7b", name: "Code Llama 7B (Local)", reasoning: false, input: ["text"], contextWindow: 128_000, maxTokens: 4096 },
  { id: "mistral:7b", name: "Mistral 7B (Local)", reasoning: false, input: ["text"], contextWindow: 128_000, maxTokens: 4096 },
  { id: "deepseek-coder:6.7b", name: "DeepSeek Coder 6.7B (Local)", reasoning: false, input: ["text"], contextWindow: 64_000, maxTokens: 4096 },
];

const NVIDIA_NIM_MODELS = [
  { id: "deepseek-v4-pro", name: "DeepSeek V4 Pro", reasoning: true, input: ["text"], contextWindow: 262_144, maxTokens: 32_768 },
  { id: "gemma-4-31b-it", name: "Gemma 4 31B Instruct", reasoning: true, input: ["text"], contextWindow: 262_144, maxTokens: 8_192 },
  { id: "kimi-k2.5", name: "Kimi K2.5", reasoning: true, input: ["text", "image"], contextWindow: 262_144, maxTokens: 32_768 },
  { id: "glm-5.1", name: "GLM-5.1", reasoning: true, input: ["text"], contextWindow: 128_000, maxTokens: 8_192 },
  { id: "minimax-m2.7", name: "MiniMax M2.7", reasoning: true, input: ["text", "image"], contextWindow: 128_000, maxTokens: 8_192 },
];
...
export default async function (pi: ExtensionAPI) {
  ...
  pi.registerTool({
    ...
    async execute(toolCallId, params, signal, onUpdate, ctx) {
      ...
      // Delegate to pi's built-in tools for actual file scanning
      // This is a coordination wrapper
      return {
        content: [
          {
            type: "text",
  ...
  pi.registerTool({
    ...
    async execute(toolCallId, params, signal, onUpdate, ctx) {
      ...
      // Delegate to pi's web search capabilities
      return {
        content: [
          {
            type: "text",
